...
@section('title', 'Trashed Teachers')
@section('mine-title', 'Teachers Management')
@section('sub-title', 'Trashed Teachers')
...
<style>
    ...
    .email-badge { background: linear-gradient(135deg, #11998e, #38ef7d); color: white; padding: 0.35rem 0.75rem; border-radius: 15px; font-size: 0.78rem; font-weight: 700; letter-spacing: 0.3px; display: inline-block; }
        ...
        <div class="col-md-12">
            <div class="card shadow-sm">
                ...
                <div class="card-header-gradient d-flex justify-content-between align-items-center">
                    <h3><i class="fas fa-trash mr-2"></i> Trashed Teachers</h3>
                    <div style="display: flex; gap: 10px;">
                        <form action="{{ route('teachers.restore-all') }}" method="POST" class="d-inline"
                              onsubmit="return confirm('هل تريد استعادة كل المدرسين؟')">
                            @csrf
                        ...
                        <a href="{{ route('teachers.index') }}" class="btn-add">
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="deleted-at-badge">
                                        ...
                                        {{ $teacher->deleted_at ? $teacher->deleted_at->format('Y-m-d H:i') : '-' }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center" style="gap: 5px;">
                                        <form action="{{ route('teachers.restore.post', $teacher->id) }}" method="POST" class="d-inline">
                                            @csrf
                                            <button type="submit" class="btn btn-action" style="background:#d4edda; color:#1e7e34;" title="استعادة">
                                                <i class="fas fa-undo"></i>
                                            </button>
                                        </form>
                                        <a href="{{ route('teachers.show', $teacher->id) }}" class="btn btn-action" style="background:#d1ecf1; color:#0c7db1;" title="عرض">
                                            <i class="fas fa-eye"></i>
                                        </a>
                                        <form action="{{ route('teachers.force-delete', $teacher->id) }}" method="POST" class="d-inline"
                            ...
                            <tr>
                                <td colspan="4" class="text-center py-4">
                                    ...
                                    There are no teachers in trash.
                                </td>
                            </tr>
                            @endforelse
                        </tbody>
