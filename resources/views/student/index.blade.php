...
@section('sub-title', 'All Students')
...
<style>
    ...
    .table tbody tr:hover { background-color: #f0f4ff !important; }
    .table tbody td {
        ...
        <div class="col-md-12">
            <div class="card shadow-sm">
                ...
                <div class="card-header-gradient d-flex justify-content-between align-items-center">
                    <h3><i class="fas fa-user-graduate mr-2"></i> Students List</h3>
                    <div style="display: flex; gap: 10px;">
                        <a href="{{ route('students.create') }}" class="btn-add">
                            <i class="fas fa-plus mr-1"></i> Create New Student
                        </a>
                        <form action="{{ route('students.delete-all') }}" method="POST" class="d-inline"
                        ...
                        <a href="{{ route('students.trashed') }}" class="btn-add">
                ...
                <div class="card-body p-0">
                    <form action="{{ route('students.index') }}" method="GET" class="p-3 border-bottom">
                        <div class="row">
                            ...
                            <div class="col-md-2 mb-3 d-flex align-items-end">
                                ...
                                <a href="{{ route('students.index') }}" class="btn btn-light">Reset</a>
                            </div>
                        </div>
                    </form>
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td><span class="student-name">{{ $student->last_name ?? '-' }}</span></td>
                                    <td>{{ $student->email ?? '-' }}</td>
                                    <td class="text-center">
                                        <span class="info-badge gender-badge">{{ strtolower($student->gender ?? 'male') }}</span>
                                    </td>
                                    <td class="text-center">
                                        <span class="info-badge status-badge {{ strtolower($student->status ?? 'inactive') === 'active' ? 'status-active' : 'status-inactive' }}">
                                            {{ strtolower($student->status ?? 'inactive') }}
                                        </span>
                                    </td>
                                    <td class="text-center">
                                        <span class="info-badge country-badge">{{ $student->country?->name ?? '-' }}</span>
                                    </td>
                                    <td class="text-center">
                                        <div class="settings-group">
                                            <a href="{{ route('students.edit', $student->id) }}" class="btn-setting btn-edit">Edit</a>
                                            ...
                                            <a href="{{ route('students.show', $student->id) }}" class="btn-setting btn-show">Show</a>
                                        </div>
                                    </td>
                                </tr>
                                @empty
                                <tr>
                                    <td colspan="8" class="text-center py-4">There are no Students yet.</td>
                                </tr>
                                @endforelse
                            </tbody>
                        </table>
