@extends('cms.perant')
@section('title', 'Trashed Lessons')
@section('mine-title', 'Lessons Management')
@section('sub-title', 'Trashed Lessons')
@section('styles')
<style>
        ...
        <div class="col-md-12">
            <div class="card shadow-sm">
                <div class="card-header-gradient d-flex justify-content-between align-items-center">
                    <h3><i class="fas fa-trash mr-2"></i> Trashed Lessons</h3>
                    <div style="display:flex; gap:10px; align-items:center;">
                        <a href="{{ route('lessons.index') }}" class="btn-add" style="background: #ecf0f1; color: #333;">
                        ...
                        <table class="table table-hover mb-0">
                            ...
                            <tbody>
                                @forelse($lessons as $lesson)
                                    <tr>
                                        <td class="text-center"><span class="id-badge">{{ $lesson->id }}</span></td>
                                        <td>{{ $lesson->title }}</td>
                                        <td>{{ $lesson->course->title ?? '-' }}</td>
                                        <td>{{ $lesson->deleted_at ? $lesson->deleted_at->format('Y-m-d H:i') : '-' }}</td>
                                        <td class="text-center">
                                            <div class="d-flex justify-content-center" style="gap:5px;">
                                                <form action="{{ route('lessons.restore', $lesson->id) }}" method="POST" class="d-inline">
                                                    @csrf
                                                    <button type="submit" class="btn-action" style="background:#d4edda; color:#1e7e34;" title="Restore">
                                                        <i class="fas fa-undo"></i>
                                                    </button>
                                                </form>
                                                <a href="{{ route('lessons.show', $lesson->id) }}" class="btn-action" style="background:#d1ecf1; color:#0c7db1;" title="Show">
                                                    <i class="fas fa-eye"></i>
                                                </a>
                                                <form action="{{ route('lessons.force-delete', $lesson->id) }}" method="POST" class="d-inline" onsubmit="return confirm('هل تريد حذف الدرس نهائياً؟');">
                                                    @csrf
                                ...
                                @empty
                                    <tr>
                                        <td colspan="5" class="text-center py-4">No trashed lessons available.</td>
                                    </tr>
                                @endforelse
                            </tbody>
                        </table>
