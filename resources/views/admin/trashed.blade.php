...
@section('sub-title', 'Trashed Admins')
        ...
        <div class="col-md-12">
            <div class="card shadow-sm">
                ...
                <div class="card-header-gradient d-flex justify-content-between align-items-center">
                    <h3><i class="fas fa-trash mr-2"></i> Trashed Admins</h3>
                    <div style="display: flex; gap: 10px;">
                        <form action="{{ route('admins.restore-all') }}"
                              method="POST"
                              class="d-inline"
                              onsubmit="return confirm('هل تريد استعادة كل المسؤولين؟')">
                            @csrf
                        ...
                        <a href="{{ route('admins.index') }}" class="btn-add">
                            ...
                            <tr>
                                ...
                                <td class="text-center">
                                    <span class="deleted-at-badge">
                                        ...
                                        {{ $admin->deleted_at ? $admin->deleted_at->format('Y-m-d H:i') : '-' }}
                                    </span>
                                </td>
                                <td class="text-center">
                                    <div class="d-flex justify-content-center" style="gap: 5px;">
                                        {{-- Restore --}}
                                        <form action="{{ route('admins.restore.post', $admin->id) }}"
                                              method="POST"
                                              class="d-inline">
                                            @csrf
                                            <button type="submit"
                                        ...
                                        {{-- Force Delete --}}
                                        <form action="{{ route('admins.force-delete', $admin->id) }}"
                                              method="POST"
                                              class="d-inline"
